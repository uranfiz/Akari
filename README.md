...
1. **Команды только от владельца.** `owner_id` в конфиге. Даже если кто-то узнает твой юзернейм — команды от него не сработают.
...
4. **Секреты не в git.** `config.yaml` и `akari.db` в `.gitignore`.
...
- Не давай никому доступ к серверу с `akari.db` — это сессия твоего аккаунта.
